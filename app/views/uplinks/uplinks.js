		...
		[ '$scope', 'uplinksApi', '$location', 'uplinksSelectionSvc', '$timeout', 
			function($scope, uplinksApi, $location, uplinksSelectionSvc, $timeout  ) {
				...
				$scope.uplinksGridOptions = {
					...
					exporterCsvFilename: ':uplinkid.csv',
    				...
    				onRegisterApi: function(gridApi){ 
      					...
      					gridApi.edit.on.afterCellEdit($scope,function(rowEntity, colDef, newValue, oldValue){
            				...
            				uplinksApi.update(req, rowEntity).$promise.then(function(success){
				...
				$scope.deleteRow = function(row) {
					...
					uplinksApi.delete(req).$promise.then(function(success){
						for (let i=0; i<$scope.uplinks.length; i++){
							if ($scope.uplinks[i].id == row.entity.id) {
								$scope.uplinks.splice(i, 1);
								refresh();
								break;
							}
						}
