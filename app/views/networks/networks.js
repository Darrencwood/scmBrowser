...
.controller('networksCtrl',
		[ '$scope', 'apiNetworks', '$location', 'currentNetworks', 
			function($scope, apiNetworks, $location, currentNetworks) {
				...
				$scope.networksGridOptions = {
					...
					data: $scope.networks,
					rowTemplate: '<div ng-dblclick="grid.appScope.clickNetworks(row)" ng-repeat="(colRenderIndex, col) in colContainer.renderedColumns track by col.uid" class="ui-grid-cell" ng-class="col.colIndex()" ui-grid-cell></div>',
					importerDataAddCallback: function( grid, newObjects ) {
      			$scope.networks = $scope.networks.concat( newObjects );
      			console.log(newObjects);
    			},
    			importerObjectCallback: function ( grid, newObject ) {
    				apiNetworks.save(newObject);
    			},
				...
				$scope.clickNetworks = function(row){ 
					//currentNetworks.setNetworks(row.entity.id);
					console.log(row);
					$scope.networksSelected = row.entity;
				}
				
				$scope.isSelected = function() {
					return ($scope.networksSelected === '')? false: true;
				}
